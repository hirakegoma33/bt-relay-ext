// bt-relay-xcratch.js  — Xcratch URL読み込み用（ESM・ファクトリ形式）
// 重要: export default function (Scratch) { return class ... } 形式
export default function (Scratch) {
  ...
  // ---- ユーティリティ ----
  const setError = (e)=> { lastError = (e && e.message) ? e.message : String(e || ''); };
  const clearError = ()=> { lastError = ''; };
  ...
  const stopReconnectTimer = ()=> { if (reconnectTimer){ clearTimeout(reconnectTimer); reconnectTimer = null; } };
  ...
  function attachDevice(dev){
    ...
    if (dev) known.set(dev.id, { id: dev.id, name: dev.name || '', device: dev });
  }
  ...
  const findById = (id)=> (known.get(id)?.device || null);
  const listIds   = ()=> Array.from(known.keys());
  const listNames = ()=> Array.from(known.values()).map(v => v.name || '');
  ...
  async function connectSendById(id, action){
    ...
    switch ((action||'').toUpperCase()){
      ...
      case 'READ':
      default:       await readState();    break;
    }
  }

  // ===== Extension class =====
  return class BtRelayExtension {
    ...
    // 接続系
    async btnConnect(){ clearError(); wantReconnect=true; stopReconnectTimer(); try{ await requestAndConnect(); }catch(e){ setError(e); scheduleReconnect(); throw e; } }
    ...
    async forgetById(args){ const id = String(args.id || '').trim(); if (device && device.id === id) await this.forgetCurrent(); else known.delete(id); }
